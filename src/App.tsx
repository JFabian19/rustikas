...
const STORES = [
  { id: 'morro-arica', name: 'Morro de Arica 3 Mz / Block R - Huamanga, Ayacucho, Peru, R\'1, Ayacucho 05003', shortName: 'Local 1 - Morro de Arica' },
  { id: 'local-2', name: 'Local 2 - Ayacucho', shortName: 'Local 2' },
  { id: 'local-3', name: 'Local 3 - Ayacucho', shortName: 'Local 3' },
];
...
export default function App() {
  ...
  return <div className="site-shell"><div className="menu-page">
    ...
    <section id="inicio" className="hero-section"><img className="hero-image" src="/hero-rustikas.jpg" alt="Pollo a la brasa Rustikas sobre brasas" /><div className="hero-shade" /><div className="hero-content"><div className="hero-kicker"><Flame size={16} fill="currentColor" /> Desde la brasa a tu mesa</div><h1>El sabor que<br /><em>enciende</em> el antojo.</h1><p>Pollo jugoso, piel crocante y el sabor inconfundible de Rustikas.</p><button className="hero-cta" onClick={() => selectCategory('menu-brasa')}>Ver promo Menú Brasa <ChevronRight size={18} /></button><div className="hero-social-links" aria-label="Encuéntranos en redes y ubicación"><span>Síguenos</span><div><a className="hero-social facebook-link" href="https://www.facebook.com/profile.php?id=61551461395500&locale=es_LA#" target="_blank" rel="noreferrer" aria-label="Visitar Facebook de Rustikas"><img src="/facebook-logo.webp" alt="" /></a><a className="hero-social maps-link" href="https://www.google.com/maps/place/Rustikasa+(pollos+y+parrillas)/@-13.1793237,-74.2106207,17z/data=!3m1!4b1!4m6!3m5!1s0x9112870068d2b2e9:0x85f9c61ef4544fa9!8m2!3d-13.1793289!4d-74.2080458!16s%2Fg%2F11z59gwjj_!5m1!1e1?entry=ttu&g_ep=EgoyMDI2MDkxNi4wIKXMDSoASAFQAw%3D%3D" target="_blank" rel="noreferrer" aria-label="Ver ubicación de Rustikas en Google Maps"><img src="/maps-logo.webp" alt="" /></a></div></div></div><div className="hero-stamp"><span>HECHO</span><strong>AL FUEGO</strong><span>CON SABOR</span></div></section>
    ...
    <footer className="site-footer"><img src="/logo-rustikas.png" alt="Rustikas Pollería & Grill" /><p>Pollería · Chifa · Restaurant</p><a href={`tel:${PHONE_DISPLAY.replace(/\s/g, '')}`}><Phone size={16} /> Pedidos: {PHONE_DISPLAY}</a><span>© 2026 Rustikas. Todos los derechos reservados.</span></footer>
  </div>
  ...
  <AnimatePresence>{showCheckout && <motion.div className="modal-backdrop checkout-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><motion.section className="checkout-panel" initial={{ scale: 0.96, y: 18 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.96, y: 18 }} role="dialog" aria-modal="true" aria-labelledby="checkout-title"><button className="close-button" onClick={() => setShowCheckout(false)} aria-label="Cerrar"><X size={20} /></button><div className="delivery-icon"><Flame size={31} fill="currentColor" /></div><p className="panel-eyebrow">CASI LISTO</p><h2 id="checkout-title">¿Cómo recibes tu pedido?</h2><div className="fulfillment-tabs"><button className={fulfillment === 'delivery' ? 'active' : ''} onClick={() => setFulfillment('delivery')}><MapPin size={17} /> Delivery</button><button className={fulfillment === 'pickup' ? 'active' : ''} onClick={() => setFulfillment('pickup')}><ShoppingBag size={17} /> Recoger en tienda</button></div><div className="checkout-fields"><label>Nombre<input value={customer.name} onChange={(event) => setCustomer({ ...customer, name: event.target.value })} placeholder="Tu nombre" autoComplete="name" /></label>{fulfillment === 'delivery' ? <><div className="store-selector-group"><label className="store-group-label">¿Desde qué local deseas que te llegue el pedido?</label><div className="store-cards">{STORES.map((st) => <label key={`del-${st.id}`} className={`store-card ${selectedStore === st.id ? 'active' : ''}`}><input type="radio" name="deliveryStore" value={st.id} checked={selectedStore === st.id} onChange={() => setSelectedStore(st.id)} /><div className="store-card-text"><strong>{st.shortName}</strong><small>{st.name}</small></div></label>)}</div></div><label>Dirección<input value={customer.address} onChange={(event) => setCustomer({ ...customer, address: event.target.value })} placeholder="Av., calle, número, distrito…" autoComplete="street-address" /></label><label>Referencia <small>Opcional</small><input value={customer.reference} onChange={(event) => setCustomer({ ...customer, reference: event.target.value })} placeholder="Ej.: portón negro, frente al parque…" /></label><div className={`location-card ${locationError ? 'has-error' : ''}`}><p><strong>Comparte tu ubicación en tiempo real</strong>Dale clic al botón y acepta los permisos para compartir tu ubicación con el restaurante.</p><button type="button" onClick={requestLocation}><LocateFixed size={17} /> {location ? 'Ubicación agregada' : locationError ? 'Volver a intentar' : 'Obtener ubicación en tiempo real'}</button>{locationStatus && <small>{locationStatus}</small>}</div></> : <><label>Teléfono<input value={customer.phone} onChange={(event) => setCustomer({ ...customer, phone: event.target.value })} placeholder="Tu número de celular" inputMode="tel" autoComplete="tel" /></label><div className="store-selector-group"><label className="store-group-label">¿En qué tienda deseas recoger tu pedido?</label><div className="store-cards">{STORES.map((st) => <label key={`pick-${st.id}`} className={`store-card ${selectedStore === st.id ? 'active' : ''}`}><input type="radio" name="pickupStore" value={st.id} checked={selectedStore === st.id} onChange={() => setSelectedStore(st.id)} /><div className="store-card-text"><strong>{st.shortName}</strong><small>{st.name}</small></div></label>)}</div></div></>}</div><fieldset className="payment-methods"><legend>Método de pago</legend>{['Yape', 'Tarjeta', 'Efectivo'].map((method) => <label key={method}><input type="radio" name="payment" value={method} checked={customer.payment === method} onChange={() => setCustomer({ ...customer, payment: method })} />{method}</label>)}</fieldset><div className="checkout-total"><span>Total estimado {deliveryFee > 0 && <small>Incluye S/ {deliveryFee.toFixed(2)} de delivery</small>}</span><strong>S/ {total.toFixed(2)}</strong></div><button className="whatsapp-button" disabled={!checkoutReady} onClick={sendToWhatsApp}><MessageCircle size={20} /> Enviar pedido</button></motion.section></motion.div>}</AnimatePresence>
  </div>;
}
