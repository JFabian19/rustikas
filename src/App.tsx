...
const isWideImageDish = (dish?: Dish, categoryId?: string) => {
  if (!dish || !categoryId) return false;
  if (categoryId === 'menu-brasa' || categoryId === 'chifa') return true;
  const text = `${dish.nombre} ${dish.descripcion || ''}`.toLowerCase();
  return text.includes('chifa') || text.includes('chaufa') || text.includes('1/2') || text.includes('medio') || text.includes('1/4') || text.includes('cuarto');
};
